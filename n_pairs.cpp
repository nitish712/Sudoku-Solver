#include "basic.cpp"
class NakedPairs : public BasicOperations
{
	...
	while(degree<=5)
	{
		for(int k=0; k<2; ++k)
		{
			...
			for(int i=0; i<9; ++i)
			{
				...
				{
					tmp_arr[j] = ((k==0)?
		make_pair(game->chance_board[i][j],	&(game->chance_board[i][j])):
		make_pair(game->chance_board[j][i],	&(game->chance_board[j][i])));
				}
				changed=0;
...
void NakedPairs::check_recur_n(int st, int cnt, int mx_cnt, int mx_idx, int val)
{
	if(cnt==mx_cnt)
	{
